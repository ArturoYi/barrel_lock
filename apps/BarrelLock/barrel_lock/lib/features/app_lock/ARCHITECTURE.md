...
| 方案 | 优点 | 缺点 |
|------|------|------|
| **A. 保留在 `barrel_lock`**（当前） | 与 `ThemedApp` Shell SSOT 同包；六平台零重复装配 | `barrel_lock` 依赖 Flutter Widget；平台 UI 定制需 override 或 fork |
...
1. Overlay 是 **Shell 基础设施**，不是路由页 View；与 `ThemedApp`、`AppLockSessionLifecycleBinder` 同属应用壳层。
...
3. `ThemedApp` 的 overlay builder 改 import `barrel_lock_ui`
