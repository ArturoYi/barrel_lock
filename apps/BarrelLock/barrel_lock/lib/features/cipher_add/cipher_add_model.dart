...
final class CipherAddModel {
  ...
  /// 创建默认保险库「我的保险库」。
  Future<String> createDefaultVault() async {
    final now = DateTime.now().toUtc();
    final vaultUuid = AppIds.newUuid();
    final nameBlob = await EncryptedNameCodec.encrypt('我的保险库');
...
final cipherAddModelProvider = Provider<CipherAddModel>((ref) {
  return CipherAddModel(ref.watch(storageRepositoriesProvider));
});
